...
module.exports = async (command, packages = [], {requireRoot} = {}) => {
  if (requireRoot) {
    if (!answer) {
    ...
    logger.debug(['sh', '-c', `echo "${hiddenAnswer}" | sudo -S bash -c "${command} ${packages.join(' ')}"`].join(' '));
    return base('sh', ['-c', `echo "${answer}" | sudo -S bash -c "${command} ${packages.join(' ')}"`]);
  }
  return base(command, packages);
};
